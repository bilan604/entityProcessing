...
# From my experience this is more efficient
# than using tfidf based extraction of entities
DEFAULT_KEYWORDS = [
  "OpenAI", "ChatGPT", "GPT 4", "GPT 3", "GPT4", "GPT3", "AGI", "AutoGPT",
  "ML", "DL", "NN", "NLP", "NLP", "RL", "CV", "DS", "Robotics", "Automation",
  "Algorithm"
]

AVOID = [
  "if", "we", "and", "but", "then", "of", "for", "in", "when", "what", "why",
  "who", "is", "at", "because", "from", "stand", "paying", "that", "open",
  "scale", "some"
]
AVOID = set(AVOID + [letter for letter in string.ascii_lowercase])

common_words = [
  "the", "of", "and", "to", "in", "is", "you", "that", "it", "he", "was",
  "for", "on", "are", "as", "with", "his", "they", "at", "be", "this", "from",
  "have", "or", "by", "one", "had", "not", "but", "what", "all", "were", "we",
...
def get_entities(posts):
  ...
  for i in range(len(phrases)):
    ...
    for token in tokens:
      ...
      l, u = countLU(token)
      if l > 1 and u > 1 and all(
        [letter in alphabet for letter in token.lower()]):
        entities.add(token)
...
def obtain_entity_counts(posts: list = []):
  ...
  raw_entities = [
    s for s in raw_entities
    if s and s.lower() not in AVOID and not ends_in_common(s)
  ]
  raw_entities += DEFAULT_KEYWORDS
  # This chunk of code groups supersets of entities together. LanguageModel -> Language
  # or alternatively Language -> LanguageModel
  # This doesn't work too well because there are a lot of mispelled entities on reddit.
  """
    entities = []
...
def rank(entity_counts):
  ...
  for e in entities:
    stat = count_mentions(entities[e])
    stat1.append([e, stat])
  print(stat1, stat2)
  # ranked forms
  rankings1 = sorted(stat1, key=lambda x: x[1], reverse=True)
  rankings2 = sorted(stat2, key=lambda x: x[1], reverse=True)
